...
from Jaatone import app
import aiofiles
from Jaatone import sex as gf
...
if STRING:
    from Jaatone  import pro
    print("App imported from Jaatone .")
else:
    pro = None
...
async def upload_media(sender, target_chat_id, file, caption, edit, topic_id):
    try:
        ...
        width, height, duration = metadata['width'], metadata['height'], duration = metadata['duration']
        try:
...
async def clone_message(app, msg, target_chat_id, topic_id, edit_id, log_group):
    ...
    Jaatonein = await app.send_message(target_chat_id, msg.text.markdown, reply_to_message_id=topic_id)
    await Jaatonein.copy(log_group)
    await edit.delete()
...
async def clone_text_message(app, msg, target_chat_id, topic_id, edit_id, log_group):
    ...
    Jaatonin = await app.send_message(target_chat_id, msg.text.markdown, reply_to_message_id=topic_id)
    await Jaatonein.copy(log_group)
    await edit.delete()
...
async def get_final_caption(msg, sender):
    ...
    if msg.caption:
        ...
        # Automatically remove Index: and any following number
        original_caption = re.sub(r"(?i)Index:\s*\d+\s*\n*", "", original_caption)
    else:
        original_caption = ""
